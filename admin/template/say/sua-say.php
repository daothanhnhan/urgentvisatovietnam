<?php 
	...
	function say ($id) {
		...
		if (isset($_POST['add_trademark'])) {
			...
			$sql = "UPDATE say SET name = '$name', day = '$day', note = '$note', link = '$link' $add WHERE id = $id";
			$result = mysqli_query($conn_vn, $sql);
			if ($result) {
				echo '<script type="text/javascript">alert(\'Bạn đã sửa được một khách nói.\')</script>';
			} else {
	...
	$info = $action->getDetail('say', 'id', $_GET['id']);
?>
...
<form action="" method="post" enctype="multipart/form-data">
    <div class="rowNodeContentPage">
        ...
        <div class="boxNodeContentPage">
            ...
            <input type="date" class="txtNCP1" name="day" value="<?= $info['day'] ?>" required/>
