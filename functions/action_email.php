...
class action_email 
{
        function email_send ($email_to, $title = "", $content = "") {
            ...
            $info = $action->getDetail('info_email', 'id', '1');
                $nFrom = $info['name'];    //mail duoc gui tu dau, thuong de ten cong ty ban
                $mFrom = $info['email'];  //dia chi email cua ban 
                $mPass = $info['pass'];       //mat khau email cua ban
                ...
                $mail->Host       = $info['smtp'];    // sever gui mail.
                $mail->Port       = 587;         // cong gui mail de nguyen
                // xong phan cau hinh bat dau phan gui mail
                $mail->Username   = $mFrom;  // khai bao dia chi email
                $mail->Password   = $mPass;              // khai bao mat khau
                $mail->SetFrom($mFrom, $nFrom);
                $mail->AddReplyTo('sale@urgentvisatovietnam.com', 'me'); //khi nguoi dung phan hoi se duoc gui den email nay
                $mail->Subject    = $title;// tieu de email 
                $mail->MsgHTML($content);// noi dung chinh cua mail se nam o day.
                $mail->AddAddress($mTo, $nTo);
                $mail->addCC('sale@urgentvisatovietnam.com');
        ...
        function email_send_2 ($email_to, $title = "", $content = "") {
            ...
            $info = $action->getDetail('info_email', 'id', '1');
                ...
                $mail->MsgHTML($content);// noi dung chinh cua mail se nam o day.
                // $mail->AddAddress($mTo, $nTo);
                $mail->AddAddress('sale@urgentvisatovietnam.com');
