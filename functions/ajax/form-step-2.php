<?php 
	...
	$_SESSION['step_1']['passport'] = array();
	$_SESSION['step_1']['portrait'] = array();
	...
	foreach ($_POST['address'] as $key => $val) {
		...
		if(isset($_FILES['portrait']) && $_FILES['portrait']['name'][$key] != ""){
			...
			uploadPicture($src, $d.$text_portrait.$time_pre.$_FILES['portrait']['name'][$key], $_FILES['portrait']['tmp_name'][$key]);
			$portrait = $d.$text_portrait.$time_pre.$_FILES['portrait']['name'][$key];
			$_SESSION['step_1']['portrait'][] = $portrait;
		}
		...
		if(isset($_FILES['passport']) && $_FILES['passport']['name'][$key] != ""){
			...
			uploadPicture($src, $d.$time_pre.$_FILES['passport']['name'][$key], $_FILES['passport']['tmp_name'][$key]);
			$passport = $d.$time_pre.$_FILES['passport']['name'][$key];
			$_SESSION['step_1']['passport'][] = $passport;
		}
	}
