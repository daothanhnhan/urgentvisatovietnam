...
class action_menu extends library{
    ...
    public function getMenuLanguageDetail_byId($valueCol_id,$lang){
        ...
        $sql = "SELECT * from $this->nameTable_menuLanguage where ($this->nameColIdMenu_menuLanguage = '".$valueCol_id."') and ($this->nameColType_menuLanguage = '".$lang."') limit 1";//echo $sql;
    ...
    public function showMenu_byMultiLevel_mainMenuTraiCam($list_menu,$valParent_menu = 0,$lang, $level = 0){
        ...
        if ($cate_child) {
            ...
            foreach ($cate_child as $key => $item) {
                //if((($level+1) == 2)&($item[$this->nameColID_menu] == 94 )) echo '<div class="coverSubMenu">';
                $row = $this->getMenuLanguageDetail_byId($item[$this->nameColID_menu],$lang);//var_dump($row);
                $name = $row[$this->nameColTitle_menuLanguage];
                $url = $this->setUrlFriendly_byType($row[$this->nameColIdMenu_menuLanguage],$lang);                
            ...
            if ($level == 0) {
                echo '<div class="__header-apply">
                        <a class="btn pd-8-16 btn-apply" href="/step-1" title="Apply now" target="_blank">Apply now</a>
                    </div>';
            }
            echo '</ul>';        
        } 
    ...
    /*-----------------------------*/
    public function showMenu_byMultiLevel_mainMenuTraiCam_mb($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
        ...
        if ($cate_child) {
            ...
            if ($level == 0) {
                echo '<li>';
                echo '<a href="/check-status">Check status</a>';
                echo '</li>';
                echo '<div class="__header-apply">
                        <a class="btn pd-8-16 btn-apply" href="/step-1" title="Apply now" target="_blank">Apply now</a>
                    </div>';
