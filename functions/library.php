...
class library{
	...
	/*------------- Hàm select list ----------------*/
	    ...
	    public function getList_type_visa($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
...
//            $results = $stmt->get_result();
	...
	/*--------------- Danh mục đệ quy ---------------*/
		...
		function getTypePage_byUrl($urlFriendly,$lang){
		    ...
		    /*--- Kiểm tra Url có thuộc requirement visa không? --*/
		    $type = 0;
		    $sql = "SELECT * FROM quoc_gia WHERE requirement_url = '$urlFriendly'";
		    $result = mysqli_query($conn_vn, $sql);
		    $total_record = mysqli_num_rows($result);
		    $type = $type + $total_record;		   
		    if ($type == 1) {
		    	...
		    	$row = mysqli_fetch_assoc($result);
			    $_GET['breadCrumb'] = $row['lang_productcat_name']; 
		    	return 'requirement_visa';}	

		   	/*--- Kiểm tra Url có thuộc emabassy visa không? --*/
		    $type = 0;
		    $sql = "SELECT * FROM quoc_gia WHERE emabassy_url = '$urlFriendly'";
		    $result = mysqli_query($conn_vn, $sql);
		    $total_record = mysqli_num_rows($result);
		    $type = $type + $total_record;		   
		    if ($type == 1) {
