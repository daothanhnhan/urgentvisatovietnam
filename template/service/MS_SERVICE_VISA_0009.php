<?php 
    ...
    $service_sub_1 = $action->getDetail('content_service', 'id', 24);
...
<?php 
    ...
    function visa_extension () {
        global $conn_vn;
        if (isset($_POST['visa_extension'])) {
            $src= "images/upload/";
            // $src = "uploads/";

            $original = '';
            if(isset($_FILES['original']) && $_FILES['original']['name'] != ""){

                $time = time();

                uploadPicture($src, $time.$_FILES['original']['name'], $_FILES['original']['tmp_name']);
                $original = $time.$_FILES['original']['name'];
            ...
            if(isset($_FILES['stamp']) && $_FILES['stamp']['name'] != ""){
                ...
                uploadPicture($src, $time.$_FILES['stamp']['name'], $_FILES['stamp']['tmp_name']);
                $stamp = $time.$_FILES['stamp']['name'];
            ...
            $city = mysqli_real_escape_string($conn_vn, $_POST['city']);
            $note = mysqli_real_escape_string($conn_vn, $_POST['note']);

            $sql = "INSERT INTO visa_extension_book (name, email, phone, nation, city, note, original, stamp) VALUES ('$name', '$email', '$phone', '$national', '$city', '$note', '$original', '$stamp')";
                ...
                <div class="btn-apply">
                    <div class="row">
                        <div class="col-md-12 text-center">
                            <span href="" title="" data-toggle="modal" data-target="#book-now-1">REQUEST A QUOTE</span>
                        </div>
  ...
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Request a Quote for Vietnam Visa Extension</h4>
      </div>
      <div class="modal-body">
        ...
        <form action="" method="post" accept-charset="utf-8" enctype="multipart/form-data">
        	<div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <input type="text" class="form-control" id="pwd" name="city" placeholder="Your Current Province/City" required="">
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="email">Photo of your original visa</label>
                        <input type="file" class="form-control" id="pwd" name="original" placeholder="Port of arrival" required="">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="email">Photo of your last extension stamp (if any)</label>
                        <input type="file" class="form-control" id="pwd" name="stamp" placeholder="Port of arrival" >
                    </div>
                </div>
                
                <div class="col-md-12">
                    <div class="form-group">
                        <textarea name="note" class="form-control" placeholder="Your message" rows="4"></textarea>
                    </div>
                </div>
            </div>
			  
			
			  <button type="submit" name="visa_extension" class="btn btn-primary">GIVE ME MY QUOTE!</button>
